...
pub struct MaintainingTimer(pub Instant);

pub fn game_ui(
    mut egui_context: EguiContexts,
    mut cube_settings: ResMut<CubeSettings>,
    mut maintaining_timer: ResMut<MaintainingTimer>,
    mut scramble_event: EventWriter<ScrambleEvent>,
    mut reset_event: EventWriter<ResetEvent>,
) {
    egui::Window::new("Control Plane").show(egui_context.ctx_mut(), |ui| {
        egui::Grid::new("ui_grid")
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    {
                        // 重置计时器
                        maintaining_timer.0 = Instant::now();
                    }
                });
                if cube_settings.play_mode == PlayMode::Timekeeping {
                    ui.add(egui::Label::new(format!(
                        "{}s",
                        maintaining_timer.0.elapsed().as_secs()
                    )));
                }
                ui.end_row();
