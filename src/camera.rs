...
pub fn setup_camera(mut commands: Commands) {
    commands
        .spawn(Camera3dBundle {
            //将相机放置在 (5.0, 5.0, 5.0) 的位置。看向场景的原点 (0, 0, 0),将 Vec3::Y 设为向上的方向。
...
pub fn move_camera(
    //函数处理鼠标移动事件，实现相机的平移和旋转。当鼠标左键按下时，根据鼠标移动的方向和距离，计算相应的旋转角度，使相机围绕原点旋转
                ...
                for mut transform in &mut q_camera {
                    if motion.delta.x.abs() > 0.001 {
                        ...
                        let quat = Quat::from_euler(
                            EulerRot::XYZ,
                            0.0,
                            0.0002 * -motion.delta.x * max * TAU, //与上下滑动保持同步
                            0.0,
                        );
